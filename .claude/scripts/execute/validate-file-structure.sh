...
find_agent_files() {
    ...
    echo "🔍 Finding files created by Agent $agent_num..."
    ...
    local find_exclude_paths=""
    for dir in "${ignored_dirs[@]}"; do
        find_exclude_paths+=" -o -name $dir -prune"
    done
    ...
    done < <(find . -type d \( -path "./workspace" -o -path "./plugins" \) ${find_exclude_paths} -o -type f -print0 2>/dev/null)
